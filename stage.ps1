...
<#
.SYNOPSIS
  Veille des offres de STAGE (finance) sur LinkedIn : controle de gestion, analyse
  financiere, audit, gestion des risques...
...
.DESCRIPTION
  ...
  - Ne garde que les offres de STAGE ET liees a la finance (mots-cles configurables).
  - Ecarte les annonces republiees (meme titre+entreprise+lieu sous un nouvel ID).
  - Deduplique d'un passage a l'autre (data-stage/seen.json) => met en avant les NOUVELLES offres.
  - Ecrit data-stage/latest.json, data-stage/latest.md et journalise data-stage/history.jsonl.
  - Pousse les offres dans la base Notion d'equipe "SUIVI STAGE" (si NOTION_TOKEN est defini).
  - Notification Telegram optionnelle (voir README).

  Frere jumeau de alternance.ps1, meme mecanique, config et donnees separees
  (config.stage.json / data-stage/) pour ne jamais se melanger avec le suivi alternance.

.EXAMPLE
  .\stage.ps1
  .\stage.ps1 -Since 48
  .\stage.ps1 -All -NoEnrich
  .\stage.ps1 -Demo
#>
[CmdletBinding()]
param(
...
$root     = $PSScriptRoot
$dataDir  = Join-Path $root 'data-stage'
...
$cfg = Read-Text (Join-Path $root 'config.stage.json') | ConvertFrom-Json
...
$termsStage = @($cfg.motsStage)      | ForEach-Object { Remove-Diacritics $_ }
...
$cards = @{}
if ($Demo) {
  $sp = Join-Path $root 'sample/stages.sample.json'
...
# Selection : stage (titre + entreprise) ET signal finance PRECIS dans le TITRE.
$candidates = New-Object System.Collections.Generic.List[object]
foreach ($c in $cards.Values) {
  $stageBlob = ' ' + (Remove-Diacritics ("$($c.intitule) $($c.entreprise)")) + ' '
  $titleBlob = ' ' + (Remove-Diacritics ([string]$c.intitule)) + ' '
  if (-not (Has $stageBlob $termsStage)) { continue }
...
function Build-Markdown {
  $sb = New-Object Text.StringBuilder
  [void]$sb.AppendLine("# Offres de stage - Finance / Risques / Analyse (LinkedIn)")
  [void]$sb.AppendLine()
  [void]$sb.AppendLine("_Fenetre : $hours h glissantes - genere le $($nowUtc.ToString('yyyy-MM-dd HH:mm')) UTC_")
  [void]$sb.AppendLine()
  [void]$sb.AppendLine("**$($offers.Count) offre(s)**, dont **$($new.Count) nouvelle(s)** depuis le dernier passage.")
  ...
  foreach ($g in @(
    ...
    foreach ($o in $g.items) {
      ...
      if ($o.matchFinance) { [void]$sb.AppendLine("- mot-cle : $($o.matchFinance)") }
...
if ($Json) {
  ...
  Write-Host ""
  Write-Host ("  Stage Finance / Risques / Analyse (LinkedIn) - {0} h glissantes" -f $hours) -ForegroundColor Cyan
...
if ($env:TELEGRAM_BOT_TOKEN -and $env:TELEGRAM_CHAT_ID -and $new.Count -gt 0) {
  ...
  $msg = "$($new.Count) nouvelle(s) offre(s) de stage finance / risques - LinkedIn (fenetre $hours h)`n`n$bloc"
  try {
...
# --------------------------------------------------------------------------------------
# Export Notion (base "SUIVI STAGE") - optionnel
# --------------------------------------------------------------------------------------
$notionToken = $env:NOTION_TOKEN
$notionDb    = if ($env:NOTION_STAGE_DB_ID) { ($env:NOTION_STAGE_DB_ID -replace '-', '') } elseif ($cfg.notionDbId) { ([string]$cfg.notionDbId -replace '-', '') } else { $null }
...
if ($notionToken -and $notionDb -and -not $Demo) {
  ...
  if (-not $Quiet -and -not $Json) {
    Write-Host ("  Notion 'SUIVI STAGE' : {0} offre(s) ajoutee(s)." -f $pushed) -ForegroundColor DarkGreen
  }
}
